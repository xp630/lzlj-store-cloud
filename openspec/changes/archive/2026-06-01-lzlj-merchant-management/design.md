...
LZLJ 通过 OpenAPI 调用 SaaS 服务获取商户数据，实现商户同步。

```
┌──────────────┐         ┌──────────────┐         ┌──────────────┐
│   LZLJ API   │         │  SaaS 网关   │         │   SaaS API   │
└──────┬───────┘         └──────┬───────┘         └──────┬───────┘
       │                        │                        │
       │ 1. GET /openapi/merchant/code/{code}         │
...
saas:
  api:
    enabled: true
    base-url: http://localhost:18080/api/saas-auth  # 通过网关调用
    api-key: ak_xxx
    api-secret: sk_xxx
```

**核心方法：syncMerchant(SyncMerchantDTO dto)**

```java
// 如果 dto 有 merchantCode，先从 SaaS 获取最新数据
if (StringUtils.hasText(dto.getMerchantCode())) {
    ...
    // 合并数据到 dto
    dto.merge(saasResult.getData());
}
// 统一保存（insert 或 update）
return saveMerchant(dto);
```

**幂等处理：**
- 有 merchantCode → 从 SaaS 获取 → insert 或 update
- 无 merchantCode → 直接本地创建

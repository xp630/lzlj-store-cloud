...
import com.lzlj.store.common.api.feign.UserFeignClient;
...
public class GoodsController {
    ...
    public Result<Long> create(@RequestBody GoodsVO goods) {
        ...
        try {
            var userResult = userFeignClient.getById(goods.getCreatorId());
    ...
    public Result<Map<String, Object>> getBatchGoodsWithCreators(
        ...
        // 跨服务调用 - 批量获取创建人信息
        String userIds = String.join(",", ids.stream().map(String::valueOf).toList());
        Map<String, Object> result = new java.util.HashMap<>();
        result.put("goodsList", goodsList);

        try {
            var userResult = userFeignClient.getBatchUsers(userIds);

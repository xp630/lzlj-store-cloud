 ...
 * 3. Sa-Token 拦截器进行登录校验
 */
@Slf4j
@Configuration
@RequiredArgsConstructor
public class SaasSaTokenConfig implements WebMvcConfigurer {

    private final RedisHelper redisHelper;
    private final com.lzlj.account.permission.service.PermissionService permissionService;
     ...
     * 添加 Sa-Token 登录校验拦截器
     * <p>
     * 注意：@SaCheckPermission 注解的权限校验需要配合 Sa-Token 提供的权限校验机制
     * 当前实现仅做登录校验，权限校验需要额外配置
     */
    @Override
    public void addInterceptors(InterceptorRegistry registry) {
        ...
        // 排除登录等白名单路径
        .excludePathPatterns(
                "/api/saas-auth/user/login",
                "/api/saas-auth/user/sms-login",
                "/api/saas-auth/doc.html",
                "/api/saas-auth/swagger-ui/**",
                "/api/saas-auth/v3/api-docs/**"
        );

        log.info("Sa-Token 权限拦截器已注册");
    }
    ...
    public Set<String> getUserPermissions(Long userId) {
        String key = "saas:user:permissions:" + userId;

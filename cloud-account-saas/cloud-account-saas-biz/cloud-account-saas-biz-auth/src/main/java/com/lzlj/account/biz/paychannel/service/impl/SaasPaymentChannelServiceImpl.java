...
public class SaasPaymentChannelServiceImpl implements SaasPaymentChannelService {
    ...
    public void delete(Long id) {
        SaasPaymentChannel channel = paymentChannelDao.selectById(id);
        if (channel == null) {
            throw new BusinessException(ResultCode.DATA_NOT_FOUND);
        }
    ...
    public PageResult<PaymentChannelDTO> page(PaymentChannelQueryDTO query, Integer pageNum, Integer pageSize) {
        Page<SaasPaymentChannel> page = new Page<>(pageNum, pageSize);
        LambdaQueryWrapper<SaasPaymentChannel> wrapper = new LambdaQueryWrapper<>();
        wrapper.like(StringUtils.hasText(query.getChannelName()), SaasPaymentChannel::getChannelCode, query.getChannelName())
               .eq(query.getStatus() != null, SaasPaymentChannel::getStatus, query.getStatus())
               .orderByDesc(SaasPaymentChannel::getCreateTime);

        IPage<SaasPaymentChannel> resultPage = paymentChannelDao.selectPage(page, wrapper);
    ...
    public List<PaymentChannelDTO> list() {
        LambdaQueryWrapper<SaasPaymentChannel> wrapper = new LambdaQueryWrapper<>();
        wrapper.orderByDesc(SaasPaymentChannel::getCreateTime);
        List<SaasPaymentChannel> channels = paymentChannelDao.selectList(wrapper);
    ...
    private PaymentChannelDTO convertToDTO(SaasPaymentChannel channel) {

package com.lzlj.account.biz.menu.service.impl;
...
public class SaasMenuServiceImpl implements SaasMenuService {
    ...
    public List<MenuDTO> getTree() {
        List<MenuDTO> cached = cacheService.getMenus();
        if (cached != null && !cached.isEmpty()) {
            return buildTree(cached, 0L);
        }
        List<SaasMenu> allMenus = getAllMenus();
        List<MenuDTO> dtos = allMenus.stream().map(this::convertToDTO).collect(Collectors.toList());
        if (!dtos.isEmpty()) {
            cacheService.setMenus(dtos);
        }
    ...
    public List<MenuDTO> getMyMenus() {
        ...
        if (isSuperAdmin) {
            ...
            List<MenuDTO> dtos = menus.stream().map(this::convertToDTO).collect(Collectors.toList());
            return buildTree(dtos, menus.get(0).getParentId());
        }
